...
export default async function HomePage() {
  const [podcasts, videos, resources, events, access] = await Promise.all([
  ...
  return <>
    <section className="home-hero page-shell">
      ...
      <div className={`hero-feature${featured?.cover_url ? " has-image" : ""}`} style={featured?.cover_url ? { backgroundImage: `linear-gradient(180deg,rgba(13,12,14,.08),rgba(13,12,14,.9)),url("${featured.cover_url}")` } : undefined}>
        ...
        <div className="feature-content"><span className="eyebrow">{featured ? "Featured conversation" : "A little room to grow"}</span><div className="feature-bottom">{featured ? <><h2>{featured.title}</h2>{featured.description && <ExpandableDescription className="home-feature-description" text={featured.description} label={`${featured.title} description`} />}<Link className="feature-play" href={`/content/${featured.slug}`}><span>▶</span> Listen now <b>↗</b></Link></> : <><h2>There’s a place<br />for you here.</h2><p>Real conversations for wherever you are in the becoming.</p><Link className="feature-play" href="/listen"><span>✳</span> Find your next listen <b>↗</b></Link></>}</div>
        </div>
    ...
    <div className="home-shelves page-shell"><MediaShelf title="Latest conversations" items={podcasts} href="/listen" /><MediaShelf title="Watch something real" items={videos} href="/watch" empty="Fresh stories are on their way. Check back soon." canWatchVideos={access.canWatchVideos} /></div>
