...
export default async function SavedPage() {
  ...
  const items=(data??[]).map((row:any)=>row.content).filter((item:any)=>item?.status==="published") as ContentRecord[];
  return <div className="library-page page-shell"><div className="page-intro"><span className="eyebrow eyebrow-line">All the good things you’ve kept</span><h1>Saved <em>for later.</em></h1><p>Your favorite Teens2Inspire content, all in one place.</p></div>{items.length?<div className="resource-grid">{items.map((item)=><MediaCard key={item.id} item={item}/>)}</div>:<div className="empty-library"><span className="empty-sparkle">♡</span><h2>Nothing saved yet.</h2><p>When you find something you love, save it here and come back anytime.</p><Link className="button button-primary" href="/listen">Explore Teens2Inspire <span aria-hidden="true">↗</span></Link></div>}</div>;
}
