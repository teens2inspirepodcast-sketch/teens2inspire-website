-- Remove legacy staff-wide access and close RPC exposure on trigger-only functions.
...
-- Trigger functions remain callable by triggers but not exposed as public RPC endpoints.
...
-- Role helpers are used by the application only inside RLS policies/functions.
